...
function load() {
  ...
  Object.prototype.r = function (n) {
    const output = []
    rx(n).forEach(() => output.push(this))
    return output
  }

  globalThis.c = function copy(x) {
    if (x && "fncopy" in x) {
      return x.fncopy()
    }
  ...
  Array.prototype.fncopy = function () {
    return this.map(c)
  }
  ...
  Array.prototype.r = function (n) {
    const output = []
    rx(n).forEach(() => output.push(this.slice()))
    return output
  }

  Array.prototype.rf = function (n) {
    return this.r(n).flat()
  }

  // Array.prototype.merge = function (windowSize, merger) {}
}

if (!globalThis.DID_LOAD_ILOWI) {
  load()
  // set after loading
  globalThis.DID_LOAD_ILOWI = true
}
